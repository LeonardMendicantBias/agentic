...
'''
## The refinement model is close to the original offline model as it inpaints by blocks and do not use memory of the past inpaintings
# The only difference is that we only care for the transformers memory and not the final inpainted image
# So we cut the "deconvolution/decoding" part to save time in the process
import numpy as np
import time
import math
...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, masked_frames): #No memory of the previous inpaintings, just a block of new frames
        attn_save = torch.Tensor().to(masked_frames.device)
        ...
        trans_feat, attn_save = self.transformer(trans_feat, attn_save)
        #Nothing after the transformers
        return attn_save #And we just return the transformers memory
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, input, save_tensor):
        x = input
        x = self.norm1(x)
        ...
        t = x.size()[0]
        attn = self.dropout(self.attention(x, t)) #No modifications of the attention calculation (we inpaint all frames)
        x = x + attn
        y = self.norm2(x)
        x = x + self.ffn(y)
        return x, save_tensor
