...
#Adding this class to handle multi outputs for the transformers
...
class InpaintGenerator(BaseNetwork):
    ...
    def __init__(self, init_weights=True):
        ...
        for k in range(stack_num//2): #Adding ids to the transformers, to identify then when loading inpainting memory
            blocks.append(TransformerBlock(2*k, token_size, hidden=hidden, num_head=num_head, mode='t', dropout=dropout))
            blocks.append(TransformerBlock(2*k+1, token_size, hidden=hidden, num_head=num_head, mode='s', dropout=dropout))
        self.transformer = mySequential(*blocks)
    ...
    def forward(self, masked_frames, former_attn): #Adding memory of the previous frames as input
        attn_save = torch.Tensor().to(masked_frames.device) #Adding this variable to save the inpainting of the new frame (to then save it)
        ...
        trans_feat, attn_save, _ = self.transformer(trans_feat, attn_save, former_attn) #The memory is used here in the transformers and we retrieve the new inpainting info
        trans_feat = self.vec2patch(trans_feat)
        enc_feat = enc_feat + trans_feat

        output = self.decoder(enc_feat)
        output = torch.tanh(output)
        return output, attn_save #Returns the new inpainting vectors of the transformers to be saved
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, x, t):
        ...
        if self.mode == 's': # In mode s, we trim the keys, queries and values to only deal with the last frame (the only one we want to inpaint)
            ...
            att = att.permute(0, 1, 3, 2, 4).contiguous().view(1, n, c)
        elif self.mode == 't': # In mode t, we trim the queries to only keep the last frame
            ...
            att, _ = self.attention(query, key, value)
            att = att.view(b, 2, 2, self.head, 1, self.h//2, self.w//2, c_h)
            att = att.permute(0, 4, 1, 5, 2, 6, 3, 7).contiguous().view(1, n, c)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, id, tokensize, hidden=128, num_head=4, mode='s', dropout=0.1):
        super().__init__()
    ...
    def forward(self, input, save_tensor, formers): #We add here the saving tensor (for the new inpainted frame) and the memory of the former inpaints
        x = input
        x = self.norm1(x)

        # Adding the last transformer output to the new saving vector
        # print("save_tensor", save_tensor.shape, x.shape)
        # save_tensor = torch.cat((save_tensor, x))  # .unsqueeze(0): no need anymore
        save_tensor = torch.cat((save_tensor, x.unsqueeze(0)))
        k = x.size()[1]

        #Loading memory from former frames
        if formers.numel(): #check if empty tensor or not
            load = formers[:,self.id].view(-1,k,self.hidden)
            x = torch.cat((x,load), 0)
        t = x.size()[0]

        attn = self.dropout(self.attention(x, t))
        x = x[:1] + attn
        y = self.norm2(x)
        x = x + self.ffn(y)
        return x, save_tensor, formers #Same format as the input because it passes that to the next transformer in the sequence
